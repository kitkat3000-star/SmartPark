                ...
                dREF.addSnapshotListener(getActivity(), new EventListener<DocumentSnapshot>() {
                    ...
                    public void onEvent(@Nullable DocumentSnapshot value, @Nullable FirebaseFirestoreException error) {
                        ...
                        if (leftparkinglot) {
                            ...
                            Fstore.collection("Parking Lot").document("UOWD").collection(parkingZone).document(parkingSpot)
                                    .update("status", true);
                            Fstore.collection("Parking Lot").document("UOWD").collection(parkingZone).document(parkingSpot)
                                    .update("reservationId", "");
                            GREF = Fstore.collection("Users").document(userID);
                            GREF.update("parked", pparked, "enteredParkingLot", entered);
                            try {
                                //set time in mili
                                Thread.sleep(5000);
                            ...
                            GREF.update("leftParkingLot", left);
